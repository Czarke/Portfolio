...
[KVM](https://www.linux-kvm.org/page/Main_Page) (Kernel-based Virtual Machine) is a Linux kernel module that lets the kernel act as a hypervisor. [libvirt](https://ubuntu.com/server/docs/how-to/virtualisation/libvirt/) sits on top of it and provides tooling — `virsh` for managing VMs from the CLI, storage pool management, network management.
...
My [Netplan](https://netplan.readthedocs.io/en/stable/) config on the host:
...
**Install [containerd](https://github.com/containerd/containerd/tree/main).** This is the container runtime — the thing that actually runs containers. One config change is critical: `SystemdCgroup = true`. Both systemd (Ubuntu's init system) and containerd need to manage cgroups, which I learned is the kernel feature that enforces resource limits on processes. If they use different cgroup drivers, they conflict and cause node instability. Setting containerd to use the systemd driver means they cooperate.
...
Kubernetes doesn't handle pod networking itself — it defines an interface ([CNI](https://kubernetes.io/docs/concepts/extend-kubernetes/compute-storage-net/network-plugins/)) and expects a plugin to implement it. [Flannel](https://github.com/flannel-io/flannel) is what I installed.
